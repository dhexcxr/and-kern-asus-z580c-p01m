...
static int suspend_test(int level)
{
	...
	if (pm_test_level == level) {
		printk(KERN_INFO "suspend debug: Waiting for %d ms.\n",pm_test_interval_time);
		mdelay(pm_test_interval_time);
		return 1;
	}
...
static bool doing_suspend=false;
inline bool is_suspending(void){
	return doing_suspend;
}
...
static int enter_state(suspend_state_t state)
{
	...
	doing_suspend=true;//begin going to S3
 ...
 Finish:
	...
	suspend_finish();
 Unlock:
	doing_suspend=false;//end of suspending process
