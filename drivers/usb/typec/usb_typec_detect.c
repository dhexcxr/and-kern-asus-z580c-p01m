...
#include <linux/../../drivers/power/battery/smb345_external_include.h>
...
const char* typec_detect_state_str(enum typec_detect_state s)
{
	static const char* detect_state_str[]        = {
		[DETECT_STATE_UNATTACHED_UFP]      = "DETECT_STATE_UNATTACHED_UFP",
		[DETECT_STATE_UNATTACHED_DFP]      = "DETECT_STATE_UNATTACHED_DFP",
		[DETECT_STATE_UNATTACHED_DRP]      = "DETECT_STATE_UNATTACHED_DRP",
		[DETECT_STATE_ATTACHED_DFP]        = "DETECT_STATE_ATTACHED_DFP",
		[DETECT_STATE_ATTACH_DFP_DRP_WAIT] = "DETECT_STATE_ATTACH_DFP_DRP_WAIT",
		[DETECT_STATE_LOCK_UFP]            = "DETECT_STATE_LOCK_UFP",
		[DETECT_STATE_ATTACHED_UFP]        = "DETECT_STATE_ATTACHED_UFP",
	};
	BUG_ON(s >= DETECT_STATE_INVALID);
	return detect_state_str[s];
}
EXPORT_SYMBOL_GPL(typec_detect_state_str);
...
static void detect_dfp_work(struct work_struct *work)
{
	...
	DBG_RL(DBG_SM, "++ %s: detect->state=%s, cc1_found = %d cc2_found = %d unattach dfp cc1 = %d, cc2 = %d\n",
	       __func__, typec_detect_state_str(detect->state), cc1_found, cc2_found, cc1.v_rd, cc2.v_rd );
	...
	if (cc1_found && cc2_found) {
		if (((CC_RA(cc1.v_rd) || (CC_OPEN(cc1.v_rd)))
		    ...
		    (CC_RD(cc1.v_rd) && (CC_RA(cc2.v_rd) ||
					 CC_OPEN(cc2.v_rd)))) {
			del_timer(&detect->drp_timeout_timer); /* disable timeout timer */
			...
			atomic_notifier_call_chain(&detect->otg->notifier,
						   USB_EVENT_ID, NULL);
			/* Set USB_ID (to PMIC.N11) to low, trigger OTG FSM
			 */
			if (gpio_is_valid(phy->id_gpio))
				gpio_set_value(phy->id_gpio, 0);
			typec_wait_for_vbus(phy, true);
			typec_enable_irq(phy);

			dev_info(detect->phy->dev, "OTG device Detected");
			return;
...
static void detect_drp_timeout_timer(unsigned long data)
{
	struct typec_detect *detect = (struct typec_detect *)data;
	struct typec_phy *phy;
	...
	DBG(DBG_ERR, "%s: stopping DRP & TDM\n", __func__);

	detect->event = TYPEC_EVENT_STOP_DRP;
...
static void update_phy_state(struct work_struct *work)
{
	...
	phy = detect->phy;
	DBG(DBG_SM, "++ %s: phy_state=%s, event=%s, detect state=%s\n", __func__,
	    typec_phy_state_str(phy->state), typec_event_str(event),
	    typec_detect_state_str(detect->state));
	...
	case TYPEC_EVENT_VBUS:
		...
		dev_info(detect->phy->dev, "evt_vbus cc1 = %s, cc2 = %s",
			 typec_cc_level_str(cc1_psy.v_rd),
			 typec_cc_level_str(cc2_psy.v_rd));
	...
	case TYPEC_EVENT_NONE:
		...
		if (detect->state == DETECT_STATE_ATTACHED_UFP) {
			...
			atomic_notifier_call_chain(&power_supply_notifier,
						   POWER_SUPPLY_CABLE_EVENT,
						   &cable_props);
			typec_wait_for_vbus(phy, false);
			typec_enable_irq(phy);
		} else {
			/* state = DFP; disable VBUS */
			if (setSMB345Charger(DISABLE_5V)) {
				DBG(DBG_ERR, "%s: error in disabling VBUS\n", __func__);
			}
			/* set OTG ID */
			if (gpio_is_valid(phy->id_gpio))
				gpio_set_value(phy->id_gpio, 1);
			typec_wait_for_vbus(phy, false);
			typec_enable_irq(phy);
	...
	case TYPEC_EVENT_TDM_DFP:
		...
		break;
	default:
		dev_err(detect->phy->dev, "unknown event %d", event);
	}
	DBG(DBG_SM, "-- %s: phy_state=%s, detect state=%s\n", __func__,
	    typec_phy_state_str(phy->state), typec_detect_state_str(detect->state));
}
				...
				unsigned long event, void *data)
{
	...
	case TYPEC_EVENT_TDM_DFP:
		schedule_work(&detect->phy_ntf_work);
		if (drp_timeout_ms > 0)
			mod_timer(&detect->drp_timeout_timer,
				  jiffies + msecs_to_jiffies(drp_timeout_ms));
		break;
	case TYPEC_EVENT_TDM_DRP:
...
int typec_bind_detect(struct typec_phy *phy)
{
	...
	setup_timer(&detect->drp_timeout_timer, detect_drp_timeout_timer,
		    (unsigned long)detect);
	...
	ret = extcon_dev_register(detect->edev, phy->dev);
	if (ret) {
