...
static int sdhci_do_get_cd(struct sdhci_host *host)
{
    ...
    /* Driver doesn't implement mmc/gpio framework */
	if (host->ops->get_cd) {
		sdhci_runtime_pm_get(host);
		gpio_cd = host->ops->get_cd(host);
		sdhci_runtime_pm_put(host);
        return gpio_cd;
	}
						...
						struct mmc_ios *ios)
{
	...
	case MMC_SIGNAL_VOLTAGE_330:
		...
		if (host->vqmmc) {
			ret = regulator_set_voltage(host->vqmmc, 2900000, 3300000);
			if (ret) {
...
int sdhci_add_host(struct sdhci_host *host)
{
	...
	if (IS_ERR_OR_NULL(host->vqmmc)) {
        ...
        /* core/mmc_regulator_set_ocr will try to sync the status of
         * vmmc and vqmmc */
        host->mmc->supply.vqmmc = host->vqmmc;
