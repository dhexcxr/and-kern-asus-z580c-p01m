...
static int intel_moor_emmc_probe_slot(struct sdhci_pci_slot *slot)
{
	...
	slot->host->mmc->caps2 |= MMC_CAP2_HS200_1_8V_SDR |
        MMC_CAP2_HS200_DIS | MMC_CAP2_DEFERRED_RESUME;
...
static int intel_moor_sd_probe_slot(struct sdhci_pci_slot *slot)
{
    ...
    slot->host->mmc->caps |= MMC_CAP_HW_RESET;
	slot->host->mmc->caps2 |= MMC_CAP2_FIXED_NCRC | MMC_CAP2_DEFERRED_RESUME;
	if (slot->data)
		if (slot->data->platform_quirks & PLFM_QUIRK_NO_HOST_CTRL_HW)
			ret = -ENODEV;
...
#ifdef	Z580C
	bool chip = 0;
	chip = __gpio_get_value(15);
	printk(KERN_ERR"get gpio 15 = %d\n", chip);
...
static void sdhci_pci_hw_reset(struct sdhci_host *host)
{
	...
	if (gpio_is_valid(rst_n_gpio)) {
        ...
        if (slot->chip->pdev->device == PCI_DEVICE_ID_INTEL_MOOR_SD) {
...
static ssize_t SD_status(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    if (pdev->device != PCI_DEVICE_ID_INTEL_MOOR_SD)
        return 0;
				     ...
				     const struct pci_device_id *ent)
{
    ...
    if (pdev->device == PCI_DEVICE_ID_INTEL_MOOR_SD ||
        pdev->device == PCI_DEVICE_ID_INTEL_MOOR_EMMC) {
        /* only create for eMMC and uSD */
