				    ...
				    struct mmc_async_req *areq, int *error)
{
	...
	if (!err && areq) {
#if 0
        /* disable the original one, it will be traced in mmc_request_done */
			...
			unsigned short vdd_bit)
{
	...
	if (vdd_bit) {
		...
		if (result == 0 && !mmc->regulator_enabled) {
            /* before enable vmmc. Check vqmmc at first,
             * turn it on if it is not enabled */
            if (mmc->supply.vqmmc) {
                result = regulator_enable(mmc->supply.vqmmc);
                if (result) {
        ...
        /* After disable vmmc, also disable vqmmc if it was enabled */
        if (mmc->supply.vqmmc) {
            result = regulator_disable(mmc->supply.vqmmc);
            if (result) {
                pr_warn("%s: Failed to disable vqmmc regulator: %d\n",
                        mmc_hostname(mmc), result);
            }
        }
	}
...
{
    /* In Asus implementation (mmc1), hardware reset is done by power recycle.
     * It can skip because of card init always start with VCCSDIO is absent.
     * */
    if (!(host->caps & MMC_CAP_HW_RESET) || !host->ops->hw_reset ||
        (strncmp(mmc_hostname(host), "mmc1", 4) == 0))
		return;
	mmc_host_clk_hold(host);
