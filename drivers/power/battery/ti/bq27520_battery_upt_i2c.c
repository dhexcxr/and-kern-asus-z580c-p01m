...
#ifdef BQ27520_UPDATE_FORCE_CHECKING
...
int bq27520_bat_upt_main_update_flow(void)
{
    ...
    BATT_ID = sd_cv_batt_id_resistor();
    BAT_DBG("Battery ID: %dk Ohms\n", BATT_ID);

#if defined(CONFIG_Z580C)
    if(BATT_ID >= 90000 && BATT_ID <= 110000)
    {
        BAT_DBG("cell type: ATL\n");
        bq27xx_fw[0].op = bq27xx_fw_ATL[0].op;
        curr_cell = BATT_ATL;
    }
    else if(BATT_ID >= 46000 && BATT_ID <= 56000)
    {
    ...
    {
        BAT_DBG("cell type: Unknow\n");
        return UPDATE_BATT_ID_FAIL;
    }
#endif
