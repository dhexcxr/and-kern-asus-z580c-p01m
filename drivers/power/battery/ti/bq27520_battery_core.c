...
#define BATT_LGC 0x314C
#define BATT_ATL 0x3141
...
#if defined(CONFIG_Z580C)
	if(HW_ID == HW_ID_ER)
		ret = bq27520_read_i2c(client, 0x20, &rsoc, 0);
	else
		ret = bq27520_read_i2c(client, BQ27520_REG_SOC, &rsoc, 0);
#else
    ret = bq27520_read_i2c(client, BQ27520_REG_SOC, &rsoc, 0);
...
int bq27520_asus_battery_dev_read_percentage(void)
{
    ...
    do{
        ...
        if (ret == 0)
        {
            BAT_DBG("Read percentage 0%, retry!\n");
        }
        retry--;
        udelay(800);
    }while(ret == 0 && retry > 0);
...
int bq27520_dump_registers(struct seq_file *s)
{
    ...
    ret = bq27520_read(client, 0x00, &temp_buf, 0);
    if(ret)
        seq_printf(s, "Control Status(): error\n");
    else
        seq_printf(s, "Control Status(): %04X\n", temp_buf);

	ret = bq27520_read(client, 0x0A, &temp_buf, 0);
    if(ret)
        seq_printf(s, "Battery Status(): error\n");
    else
        seq_printf(s, "Battery Status(): %04X\n", temp_buf);

    ret = bq27520_read(client, 0x3A, &temp_buf, 0);
    if(ret)
        seq_printf(s, "Operation Status(): error\n");
    else
        seq_printf(s, "Operation Status(): %04X\n", temp_buf);

    do
    {
    ret = bq27520_cntl_cmd(client, 0x0056);
    if (ret) {
        dev_err(&client->dev, "Send subcommand 0x%04X error.\n", 0x0056);
        retry_count--;
        msleep(I2C_RETRY_DELAY);
    ...
    do
    {
    ret = bq27520_read(client, 0x3E, &temp_buf, 0);
    if (ret)
    {
        dev_err(&client->dev, "Error!! %s subcommand %04X\n",
                         __func__, 0x0056);
    ...
    if(ret)
        seq_printf(s, "Gauging Status(): error\n");
    else
        seq_printf(s, "Gauging Status(): %04X\n", temp_buf);
...
static int fwupdate_debugfs_show(struct seq_file *s, void *data)
{
    int ret;
    ret = bq27520_bat_upt_main_update_flow();

    if (ret < 0)
        seq_printf(s, "Gauge FW update Fail\n");
    else if(ret == 0)
        seq_printf(s, "Gauge FW not need to update\n");
    else
        seq_printf(s, "Gauge FW update Success\n");

    asus_update_all();
    return 0;
}

static int fwupdate_debugfs_open(struct inode *inode, struct file *file)
{
    return single_open(file, fwupdate_debugfs_show, inode->i_private);
}

static const struct file_operations fwupdate_debugfs_fops = {
    .open = fwupdate_debugfs_open,
    .read = seq_read,
    .llseek = seq_lseek,
    .release = single_release,
};

static int fwversion_debugfs_show(struct seq_file *s, void *data)
{
    int fw_cell;
    int DF_ver;
    ...
    if(fw_cell == BATT_LGC)
        seq_printf(s, "LGC-%04X\n", DF_ver);
    if(fw_cell == BATT_ATL)
        seq_printf(s, "ATL-%04X\n", DF_ver);

    return 0;
}

static int fwversion_debugfs_open(struct inode *inode, struct file *file)
{
    return single_open(file, fwversion_debugfs_show, inode->i_private);
}

static const struct file_operations fwversion_debugfs_fops = {
    .open = fwversion_debugfs_open,
    .read = seq_read,
    .llseek = seq_lseek,
    .release = single_release,
};
                            ...
                            const struct i2c_device_id *id)
{
    ...
    debugfs_create_file("gaugefw_update", 0644, NULL, NULL, &fwupdate_debugfs_fops);
    debugfs_create_file("fw_version", 0644, NULL, NULL, &fwversion_debugfs_fops);
#endif
...
#if defined(CONFIG_FE380CG) || defined(CONFIG_A503ML)
	ret = bq27520_bat_upt_main_update_flow();
	mutex_lock(&bq27520_dev_info_mutex);
...
//#ifdef ME372CG_USERDEBUG_BUILD
	ret = bq27520_bat_upt_main_update_flow();
	mutex_lock(&bq27520_dev_info_mutex);
	bq27520_dev_info.update_status = ret;
	mutex_unlock(&bq27520_dev_info_mutex);
	/* ignore Voltage Not Enough if triggered update
	to avoid ? battery icon shown */
