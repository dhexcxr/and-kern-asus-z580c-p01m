...
static unsigned int wifi_wakeup_count;
...
static void lnw_irq_handler(unsigned irq, struct irq_desc *desc)
{
	...
	for (base = 0; base < lnw->chip.ngpio; base += 32) {
		...
		while ((pending = (lnw->type != TANGIER_GPIO) ?
			...
			int gpio_num = base + gpio;
			if (lnw->wakeup){
				dev_info(&lnw->pdev->dev,
				"wakeup source: gpio %d\n", gpio_num);

				if (gpio_num == 64){
					if(wifi_wakeup_count>=60000)	wifi_wakeup_count=wifi_wakeup_count%60000;
					wifi_wakeup_count++;
				}else if(gpio_num == 70){
...
{
	sscanf(buf, "%du", &wifi_wakeup_count);
	return count;
}

static struct kobj_attribute wifi_attribute =
	__ATTR(wifi_count, 0644, wifi_show, wifi_store);
...
/*
static int add_file_node(const struct attribute *attr)
{
	int retval;
	retval = sysfs_add_file_to_group(wakeup_count_kobj, attr, "Alarm");
	return	retval;
}
EXPORT_SYMBOL(add_file_node);
*/
static int __init wakeup_count_init(void)
{
	...
	wakeup_count_kobj = kobject_create_and_add("connect_wakeup", kernel_kobj);
...
static void __exit wakeup_count_exit(void)
{
	//kobject_put(wakeup_count_kobj);
}
