...
//CY+
				...
				if (dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf),TRUE, 0) < 0)
					DHD_ERROR(("%s: set pm2_sleep_ret failed\n", __FUNCTION__));
//CY-
...
//FIX driver hold wlan_wake and let system unable to suspend(TCPACK_SUPRESS bug)
//			return 0;
			ret = 0;
			goto done;
		}
...
//CY+ monitor wakeup pkt
static int ENABLE_DHD_RX_DUMP = 0;
static unsigned int ENABLE_DHD_RX_DUMP_CNT = 0;
void enable_dhd_rx_dump(int in){
	if(in > 0)
		ENABLE_DHD_RX_DUMP_CNT++;

	printk("@@@@@@@@@@%s: %d, cnt:%u\n", __FUNCTION__, in, ENABLE_DHD_RX_DUMP_CNT);
	ENABLE_DHD_RX_DUMP = in;
}
EXPORT_SYMBOL(enable_dhd_rx_dump);

int need_dhd_rx_dump(){
	return ENABLE_DHD_RX_DUMP;
}
EXPORT_SYMBOL(need_dhd_rx_dump);

//input is WIFI packet, skb->data contain MAC header, don't do skb_poll()!!
void dump_rx_pkt(struct sk_buff *skb){
	if(need_dhd_rx_dump() == 0)
		return;
	...
	if(mh == NULL){
		printk("%s:   ERROR: MAC header is null\n", __FUNCTION__);
		goto exit;
	}

	//MAC layer
	uint16 mac_proto = ntoh16(mh->h_proto);
	printk("%s:   MAC: protocol: %u\n", __FUNCTION__, mac_proto);
	if(skb->len >= 14){//data contain MAC Header
		if(mh != NULL){
			printk("%s:   MAC: src=%.2x:%.2x:%.2x:%.2x:%.2x:%.2x, dst=%.2x:%.2x:%.2x:%.2x:%.2x:%.2x\n",
				__FUNCTION__,
		...
		printk("%s:   ERROR: no MAC header in skb!\n", __FUNCTION__);
		goto exit;
	}
	...
	switch(mac_proto){
		case ETHER_TYPE_IP:
			printk("%s:   MAC protocol: ETHER_TYPE_IP(IPv4)\n", __FUNCTION__);

			if(skb->len >= (14 + 20)){ //data contain MAC Header and min IPv4 Header
				struct iphdr *ih = ((char *)mh)+14;
				printk("%s:     IPv4: IP protocol=%u, check RFC790 for detail\n", __FUNCTION__, ih->protocol);
				printk("%s:     IPv4: src=%pI4, dst=%pI4\n", __FUNCTION__, &ih->saddr, &ih->daddr);
			}else{
				printk("%s:     ERROR: no IPv4 header in skb!\n", __FUNCTION__);
				goto exit;
			}

			break;
		case ETHER_TYPE_IPV6:
			printk("%s:   MAC protocol: ETHER_TYPE_IPV6\n", __FUNCTION__);

			if(skb->len >= (14 + 40)){ //data contain MAC Header and IPv6 Header
				struct ipv6hdr *ih = ((char *)mh)+14;
				printk("%s:     IPv6: src=%pI6, dst=%pI6\n", __FUNCTION__, &ih->saddr, &ih->daddr);
			}else{
				printk("%s:     ERROR: no IPv6 header in skb!\n", __FUNCTION__);
				goto exit;
			}

			break;
		case ETHER_TYPE_BRCM:
			printk("%s:   MAC protocol: ETHER_TYPE_BRCM\n", __FUNCTION__);
			break;
		case ETHER_TYPE_ARP:
			printk("%s:   MAC protocol: ETHER_TYPE_ARP\n", __FUNCTION__);
			break;
		case ETHER_TYPE_8021Q:
			printk("%s:   MAC protocol: ETHER_TYPE_8021Q\n", __FUNCTION__);
			break;
		case ETHER_TYPE_802_1X:
			printk("%s:   MAC protocol: ETHER_TYPE_802_1X\n", __FUNCTION__);
			break;
		case ETHER_TYPE_802_1X_PREAUTH:
			printk("%s:   MAC protocol: ETHER_TYPE_802_1X_PREAUTH\n", __FUNCTION__);
			break;
		case ETHER_TYPE_WAI:
			printk("%s:   MAC protocol: ETHER_TYPE_WAI(WAPI)\n", __FUNCTION__);
			break;
		case ETHER_TYPE_89_0D:
			printk("%s:   MAC protocol: ETHER_TYPE_89_0D(TDLS)\n", __FUNCTION__);
			break;
		case ETHER_TYPE_PPP_SES:
			printk("%s:   MAC protocol: ETHER_TYPE_PPP_SES(PPPoE Session)\n", __FUNCTION__);
			break;
		default:
			printk("%s:   ERROR: unknow MAC protocol\n", __FUNCTION__);
			break;
	}
...
//CY+ monitor wakeup pkt
		//only print 1 skb after resume
		if(need_dhd_rx_dump())
			enable_dhd_rx_dump(0);
//CY-
...
#ifdef CUSTOM_FW_PATH
		fw = "/factory/wifi/fw_bcmdhd.bin";
#else
		fw = CONFIG_BCMDHD_FW_PATH;
#endif /* CUSTOM_FW_PATH */
#endif /* CONFIG_BCMDHD_FW_PATH */

#ifdef CONFIG_BCMDHD_NVRAM_PATH
#ifdef CUSTOM_NV_PATH
		nv = "/factory/wifi/bcmdhd.cal";
#else
		nv = CONFIG_BCMDHD_NVRAM_PATH;
#endif /* CUSTOM_NV_PATH */
...
#ifdef GET_CUSTOM_MAC_ENABLE
	ret = wifi_platform_get_mac_addr_intel(ea_addr.octet);
	if (!ret) {
		memset(buf, 0, sizeof(buf));
...
//CY+ enable pkt_filter if in STA mode
#ifdef PKT_FILTER_SUPPORT
			dhd_pkt_filter_enable = TRUE;
#endif
//CY-
...
//CY- ASUS packet filter
	...
	if (dhd->op_mode != DHD_FLAG_HOSTAP_MODE) {
		DHD_ERROR(("STA Mode disable wlfc\n"));
		wlfc_enable = FALSE;
	} else {
		DHD_ERROR(("AP Mode enable wlfc\n"));
		wlfc_enable = TRUE;
	}
...
void dhd_get_customized_country_code(struct net_device *dev, char *country_iso_code,
	wl_country_t *cspec, uint chipnum)
#else
void dhd_get_customized_country_code(struct net_device *dev, char *country_iso_code,
	wl_country_t *cspec)
#endif
{
	...
	get_customized_country_code(dhd->adapter, country_iso_code, cspec, chipnum);
#else
...
#ifdef CHIPNUM
uint dhd_get_chipnum(struct net_device *dev){
	uint ret = 0;

	dhd_info_t *dhd = *(dhd_info_t **)netdev_priv(dev);

	if(dhd && dhd->pub.chipnum)
		ret = dhd->pub.chipnum;
