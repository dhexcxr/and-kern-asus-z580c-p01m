...
#ifdef DHD_DEBUG
	//CY+ prevent system hold wlan_wd_wake wakelock after enable WIFI FW log(dhd_console_ms)
	//             if (dhd_console_ms == 0)
#endif /* DHD_DEBUG */
		if (bus->poll == 0)
...
{
	int ret = 0;
	printf(KERN_ERR"dhdsdio_suspend!!!!!!!!!\n");
	...
	printk (KERN_ERR"dhdsdio_suspend: dhd_os_check_wakelock=%d, bus->dhd->up=%d\n", ret, bus->dhd->up);

	ret = dhd_os_check_wakelock(bus->dhd);
	if ((!ret) && (bus->dhd->up)) {

		//Force set bus->sleeping = TRUE for CHIP without SR support
		if (!SLPAUTO_ENAB(bus)){
			printk(KERN_ERR"dhdsdio_suspend: CHIP without SR support, set bus->sleeping=TRUE when suspend\n");
			bus->sleeping = TRUE;
		}

		if (wait_event_timeout(bus->bus_sleep, bus->sleeping, wait_time) == 0) {
			if (!bus->sleeping) {
				printk(KERN_ERR"dhdsdio_suspend:  bus is not sleeping, return 1\n");
				return 1;
			}
		}
	}
...
{
#if defined(OOB_INTR_ONLY)
	printf(KERN_ERR"dhdsdio_resume!!!!!!!!!\n");
	dhd_bus_t *bus = (dhd_bus_t*)context;

	//set bus->sleeping = FALSE for CHIP without SR support
	if (!SLPAUTO_ENAB(bus)){
		printf(KERN_ERR"dhdsdio_resume:  CHIP without SR support, set bus->sleeping = FALSE when resume\n");
		bus->sleeping = FALSE;
	}
