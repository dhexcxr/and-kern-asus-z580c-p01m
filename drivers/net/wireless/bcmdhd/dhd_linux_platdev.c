...
int wifi_platform_get_mac_addr_intel(unsigned char *buf){
	int ret = 0;
	int i;
	struct file *fp = NULL;
	unsigned char c_mac[MAC_ADDRESS_LEN];
	char fname[]="/factory/wifi/wifi_mac.txt";
	...
	if (!fp){
		...
		// we use random MAC address when MAC address does not write to factory/wifi_mac.txt

		buf[0]=0x08;
		buf[1]=0x00;
		buf[2]=0x28;
		get_random_bytes(&buf[3], sizeof(buf[3]));
		get_random_bytes(&buf[4], sizeof(buf[4]));
		get_random_bytes(&buf[5], sizeof(buf[5]));

		return 1;
	}

	if ( dhd_os_get_image_block(c_mac, MAC_ADDRESS_LEN, fp) != MAC_ADDRESS_LEN ){
		DHD_ERROR(("%s: Error on reading mac address from %s \n",__FUNCTION__, fname));
...
extern void enable_dhd_rx_dump(int in);
//CY-
