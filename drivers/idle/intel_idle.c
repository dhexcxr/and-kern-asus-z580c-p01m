...
static int intel_idle_probe(void)
{
	...
	if ((Read_PROJ_ID() == PROJ_ID_Z580CA) && (Read_HW_ID() == 6) ) {
		for(stateCount = max_cstate; stateCount > 0 ; stateCount--) {
			if(strcmp(cpuidle_state_table[stateCount].name,"S0i1-ATM") == 0 
				|| strcmp(cpuidle_state_table[stateCount].name,"S0i3-ATM") == 0)
				cpuidle_state_table[stateCount].disabled = true;
		}
	}
