...
extern const char* typec_phy_state_str(enum typec_state s);
...
extern const char* typec_cc_level_str(enum typec_cc_level s);
...
enum typec_event {
	...
	TYPEC_EVENT_STOP_DRP,
	TYPEC_EVENT_TDM_UFP,
	TYPEC_EVENT_TDM_DFP,
	TYPEC_EVENT_TDM_DRP,
        TYPEC_EVENT_INVALID,
};
extern const char* typec_event_str(enum typec_event s);
...
struct typec_ops {
	...
	int (*wait_for_vbus)(struct typec_phy *phy, bool rising);
	int (*mask_irq)(struct typec_phy *phy, bool enable);
};
...
struct typec_phy {
	...
	int id_gpio;                /* Since there is no ID GPIO in type.C connector,
				     * we use this pin to trigger OTG host mode. */
...
static inline int typec_enable_irq(struct typec_phy *phy)
{
	if (phy && phy->ops.mask_irq)
		return phy->ops.mask_irq(phy, 1);
	return -ENOTSUPP;
}

static inline int typec_disable_irq(struct typec_phy *phy)
{
	if (phy && phy->ops.mask_irq)
		return phy->ops.mask_irq(phy, 0);
	return -ENOTSUPP;
}

static inline int typec_wait_for_vbus(struct typec_phy *phy, bool rising)
{
	if (phy && phy->ops.wait_for_vbus)
		return phy->ops.wait_for_vbus(phy, rising);
	return -ENOTSUPP;
}
