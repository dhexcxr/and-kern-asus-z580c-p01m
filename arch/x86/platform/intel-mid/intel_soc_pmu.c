			...
			int sub_sys_index, int sub_sys_pos, pci_power_t state)
{
	...
	/* For MOFD ignore D0i0 on LSS 36 */
	if ((platform_is(INTEL_ATOM_MOORFLD)) && (sub_sys_index == 0x2))
		return (sub_sys_pos == 0x4);
	return false;
}
...
static int __init wakeup_count_init(void)
{
	...
	wakeup_count_kobj = kobject_create_and_add("alarm_wakeup", kernel_kobj);
