...
//ASUS_BSP+++
	...
	{"ov5670", RAW_CAMERA, ATOMISP_CAMERA_PORT_TERTIARY},
	...
	{"hm2051", RAW_CAMERA, ATOMISP_CAMERA_PORT_SECONDARY},
	{"gc2155", RAW_CAMERA, ATOMISP_CAMERA_PORT_TERTIARY},
//ASUS_BSP---
...
//ASUS_BSP+++
	...
	if (strcmp(idev->type, "gc2155") == 0) {
		/* Project id check. */
		switch (PROJECT_ID) {
		case PROJ_ID_FE380CXG:
			/* TODO: fe380cxg here. */
			pr_info("%s: config gc2155 as camera 0\n", __func__);
			subdev_table[i].port = ATOMISP_CAMERA_PORT_PRIMARY;
			break;
		case PROJ_ID_FE380CG:
		default:
			pr_info("%s: config gc2155 as camera 1\n", __func__);
			subdev_table[i].port = ATOMISP_CAMERA_PORT_SECONDARY;
		}
	} else if (strcmp(idev->type, "ov5670") == 0) {
		/* Project id check. */
		switch (PROJECT_ID) {
		case PROJ_ID_Z580C:
			if (offset_3 == 0) {
				pr_info("%s: This is 8M+5M china sku\n", __func__);
				pr_info("%s: config ov5670 as camera 1\n", __func__);
				subdev_table[i].port = ATOMISP_CAMERA_PORT_SECONDARY;
			} else {
				pr_info("%s: This is 5M+2M ww sku\n", __func__);
				pr_info("%s: config ov5670 as camera 0\n", __func__);
				subdev_table[i].port = ATOMISP_CAMERA_PORT_PRIMARY;
			}
			break;
		default:
