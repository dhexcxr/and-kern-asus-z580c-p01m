...
void *merfld_rt5648_audio_platform_data(void *info)
{
	struct platform_device *pdev;
	int ret;
	...
	pdev = platform_device_alloc("mrfld_rt5648", -1);
	if (!pdev) {
		pr_err("failed to allocate mrfld_rt5648 platform device\n");
		return NULL;
	}

	ret = platform_device_add(pdev);
	if (ret) {
		pr_err("failed to add mrfld_rt5648 platform device\n");
		platform_device_put(pdev);
		return NULL;
	}
	if (platform_device_add_data(pdev, &mrfld_audio_pdata,
				     sizeof(mrfld_audio_pdata))) {
		pr_err("failed to add mrfld_rt5648 platform data\n");
		platform_device_put(pdev);
		return NULL;
	}

	register_rpmsg_service("rpmsg_mrfld_rt5648_audio", RPROC_SCU,
				RP_MSIC_MRFLD_AUDIO);
